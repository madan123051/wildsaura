...
const visitorLogin = fs.readFileSync('src/components/VisitorLogin.tsx', 'utf-8');
...
assert(!/IdentityGuardWrapper/.test(mainTsx), 'public site must not require global identity verification');
assert(/Browsing, sharing, likes, and comments are free/.test(visitorLogin), 'visitor login copy must keep browsing, sharing, likes, and comments free');
assert(count(appTsx, /const actor = visitor \|\| getGuestIdentity\(\)/g) >= 3, 'photo, story, and video comments must support guest identity');
assert(/if \(!visitor\) \{ setShowVisitorLogin\(true\); return; \}/.test(appTsx), 'photo download should remain login-gated');
