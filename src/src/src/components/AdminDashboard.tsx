...
import {
  LayoutDashboard, Image, Plus, Pencil, Trash2, LogOut, Eye,
...
const StoryForm: React.FC<StoryFormProps> = ({ initial, onSave, onCancel, nextId }) => {
  ...
  const handleCoverUpload = useCallback(async (file: File) => {
    ...
    reader.onload = async () => {
      ...
      // Use data URL directly for Vercel deployment
  ...
  const handleAiFill = useCallback(async () => {
    setAiGenerating(true);
    try {
      // Step 1: If we have a cover image, use Gemini Vision to analyze it first
      let animalName = '';
      ...
      if (previewUrl && previewUrl.startsWith('data:')) {
        try {
          const { analyzePhoto } = await import('../utils/aiService');
          const result = await analyzePhoto(previewUrl);
          if (result.success && result.data) {
            animalName = result.data.animalName || '';
            detectedLocation = result.data.location || '';
            photoCaption = result.data.caption || '';
            // Auto-fill title if empty
            if (!title && result.data.title) setTitle(result.data.title);
          }
        } catch (err) {
          console.warn('Vision analysis failed for story:', err);
        }
      }

      // Step 2: Call generate-story API with Gemini
      const response = await fetch('/api/generate-story', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          photoTitle: title || 'Wildlife Photography',
      ...
      if (data.title && !slug) setSlug(data.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''));

      // Step 3: Fetch Wikipedia info if animal detected
      if (animalName) {
        try {
          const wikiRes = await fetch(`/api/wikipedia?q=${encodeURIComponent(animalName)}`);
          const wikiData = await wikiRes.json();
          if (wikiData.summary) {
            // Append Wikipedia info to content
            setContent(prev => prev + `\n\n---\n\n📚 **About ${animalName}**: ${wikiData.summary}`);
          }
        } catch {}
      }
    } catch (err) {
      console.warn('Story AI failed:', err);
      // Fallback: basic auto-fill
      if (!excerpt && title) {
        setExcerpt(`Join me on an extraordinary journey as I share the story behind "${title}" — a tale of patience, discovery, and the wild beauty that awaits those who seek it.`);
      }
      if (!tagsStr) setTagsStr('wildlife, adventure, photography, nature');
    }
    setAiGenerating(false);
  }, [title, excerpt, content, tagsStr, previewUrl, slug]);
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {aiGenerating ? (
            <><Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> 🤖 Gemini AI generating story...</>
          ) : (
            <><Sparkles size={18} /> 🤖 Gemini AI + Wikipedia Auto-Fill</>
          )}
        </button>
      </div>
