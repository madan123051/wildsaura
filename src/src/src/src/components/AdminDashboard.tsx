...
const PhotoForm: React.FC<PhotoFormProps> = ({ initial, onSave, onCancel, nextId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Upload image (Firebase Storage with localStorage fallback)
    if (imageUrl.startsWith('data:')) {
      finalImageUrl = await uploadPhotoToStorage(imageUrl, uploadedFileName || 'photo.jpg');
    }
    
    // Save metadata (Firestore with localStorage fallback)
...
const StoryForm: React.FC<StoryFormProps> = ({ initial, onSave, onCancel, nextId }) => {
  ...
  const handleAiFill = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (!content && title) {
        setContent(`The morning mist hung low over the landscape as I set out on what would become one of my most memorable photography expeditions.\n\nEvery photographer knows that the best shots come from patience and persistence. This story is no different — it took hours of waiting, careful positioning, and an intimate understanding of the environment.\n\nAs the golden light broke through, I knew the moment had arrived. The scene unfolded before me like a painting coming to life, and I pressed the shutter with practiced precision.\n\nLooking back at these images, I'm reminded why I fell in love with photography in the first place. It's not just about capturing what we see — it's about sharing what we feel.`);
      }
      setAiGenerating(false);
    }, 800);
  }, [title, excerpt, content, tagsStr]);
