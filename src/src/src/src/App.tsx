...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadPhotos = async () => {
      try {
        ...
        console.warn('Firestore load failed, using sample photos:', err);
      }
    };
    loadPhotos();
  }, []);
  ...
  const handleShare = useCallback((photo: Photo) => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(`https://wildsaura.com/post/${photo.id}`);
    }
  }, []);
  ...
  const handleAddPhoto = useCallback((photo: Photo) => { setPhotos((prev) => [photo, ...prev]); }, []);
