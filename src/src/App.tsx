...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadPhotos = async () => {
      ...
      // Try Firebase first
      try {
        let firestorePhotos = await getPhotosFromFirestore();
        // Filter out locally deleted photos
        try {
          const deletedIds: string[] = JSON.parse(localStorage.getItem('wa_deleted_ids') || '[]');
          if (deletedIds.length > 0) {
            firestorePhotos = firestorePhotos.filter(fp => !deletedIds.includes(fp.id || ''));
          }
        } catch {}
        if (firestorePhotos.length > 0) {
          const mapped = firestorePhotos.map((fp, idx) => ({
            ...
            liked: false,
            published: (fp as any).published !== false,
          }));
  ...
  const handleDeletePhoto = useCallback(async (id: number | string) => {
    ...
    // Track deleted Firestore IDs so they stay hidden on reload
    if (photo?.firestoreId) {
      try {
        const deletedIds: string[] = JSON.parse(localStorage.getItem('wa_deleted_ids') || '[]');
        if (!deletedIds.includes(photo.firestoreId)) {
          deletedIds.push(photo.firestoreId);
          localStorage.setItem('wa_deleted_ids', JSON.stringify(deletedIds));
        }
      } catch {}

      try {
        await deletePhotoFromFirestore(photo.firestoreId);
        // Success — remove from deleted list since it's gone from Firestore
        try {
          const deletedIds: string[] = JSON.parse(localStorage.getItem('wa_deleted_ids') || '[]');
          localStorage.setItem('wa_deleted_ids', JSON.stringify(deletedIds.filter(d => d !== photo.firestoreId)));
        } catch {}
      } catch (err) {
        console.warn('Firestore delete failed (hidden locally):', err);
      }
    }
  }, [photos]);
  const handleUpdatePhoto = useCallback((updated: Photo) => {
    setPhotos((prev) => {
      const newPhotos = prev.map((p) => p.id === updated.id ? updated : p);
      try {
        const userPhotos = newPhotos.filter(p => !SAMPLE_PHOTOS.find(s => s.id === p.id));
        localStorage.setItem('wa_photos', JSON.stringify(userPhotos));
      } catch {}
      return newPhotos;
    });
    if (updated.firestoreId) {
      updatePhotoInFirestore(updated.firestoreId, {
        title: updated.title, caption: updated.caption || '', category: updated.category,
        imageUrl: updated.imageUrl, location: updated.location || '', tags: updated.tags || [],
        animalName: updated.animalName || '', likeCount: updated.likeCount, type: updated.type,
        published: updated.published,
      } as any).catch(err => console.warn('Firestore update failed:', err));
    }
  }, []);
